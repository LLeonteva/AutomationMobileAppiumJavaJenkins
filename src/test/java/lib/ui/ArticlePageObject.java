...
abstract public class ArticlePageObject extends MainPageObject {
    ...
    public void ArticlePresentWithoutWait() {
        this.assertElementPresent(TITLE, "Cannot find article title in article page. Add wait?");
    }

    public void swipeToFooter() {
        if (Platform.getInstance().isAndroid()){
            this.swipeUpToFindElement(FOOTER_ELEMENT, "Cannot find the end of article", 80);
        } else {
            this.swipeUpTitleElementAppear(FOOTER_ELEMENT, "Cannot find the end of article", 80);
        }
    }
    ...
    public void addArticleToMyExistingList(String name_of_folder) {
        this.waitForElementAndClick(OPTIONS_BUTTON, "Cannot find button to open article options", 15);
        this.waitForElementAndClick(OPTIONS_ADD_TO_MY_LIST_BUTTON, "Cannot find option to add article to reading list", 15);
        this.waitForElementAndClick(("//*[contains(@text,'" + name_of_folder + "')]"), "Cannot find saved folder in overlay 'Save to reading list' ", 5);
        this.waitForElementAndClick(CLOSE_ARTICLE_BUTTON, "Cannot close article, cannot find X link", 5);
    }

    // for iOS only
    public void addArticlesToMySaved() {
        this.waitForElementAndClick(OPTIONS_ADD_TO_MY_LIST_BUTTON, "Cannot find options to add article to my reading list", 5);
    }
